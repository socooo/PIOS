...
protcseg:
  ...
  call bootmain
    7c45:	e8 c9 00 00 00       	call   7d13 <bootmain>
...
readseg(uint32_t va, uint32_t count, uint32_t offset)
{
    ...
    7cd7:	57                   	push   %edi
    7cd8:	8b 7d 10             	mov    0x10(%ebp),%edi
    7cdb:	56                   	push   %esi
    7cdc:	53                   	push   %ebx
...
readseg(uint32_t va, uint32_t count, uint32_t offset)
{
	...
	va &= 0xFFFFFF;
    7ce4:	89 de                	mov    %ebx,%esi
	end_va = va + count;
	
	// round down to sector boundary
	va &= ~(SECTSIZE - 1);
    7ce6:	81 e3 00 fe ff 00    	and    $0xfffe00,%ebx
void
readseg(uint32_t va, uint32_t count, uint32_t offset)
{
	uint32_t end_va;
	...
	va &= 0xFFFFFF;
    7cec:	81 e6 ff ff ff 00    	and    $0xffffff,%esi
	end_va = va + count;
    7cf2:	03 75 0c             	add    0xc(%ebp),%esi
	offset = (offset / SECTSIZE) + 1;
	...
	while (va < end_va) {
    7cf5:	eb 10                	jmp    7d07 <readseg+0x33>
	...
	while (va < end_va) {
		readsect((uint8_t*) va, offset);
    7d00:	e8 79 ff ff ff       	call   7c7e <readsect>
		va += SECTSIZE;
		offset++;
    7d05:	58                   	pop    %eax
    7d06:	5a                   	pop    %edx
	...
	// we load in increasing order.
	while (va < end_va) {
    7d07:	39 f3                	cmp    %esi,%ebx
    7d09:	72 ec                	jb     7cf7 <readseg+0x23>
		readsect((uint8_t*) va, offset);
		va += SECTSIZE;
		offset++;
	}
}
    7d0b:	8d 65 f4             	lea    -0xc(%ebp),%esp
    7d0e:	5b                   	pop    %ebx
    7d0f:	5e                   	pop    %esi
    7d10:	5f                   	pop    %edi
    7d11:	5d                   	pop    %ebp
...
bootmain(void)
{
    ...
    7d18:	6a 00                	push   $0x0
    7d1a:	68 00 10 00 00       	push   $0x1000
    7d1f:	68 00 00 01 00       	push   $0x10000
    7d24:	e8 ab ff ff ff       	call   7cd4 <readseg>

	// is this a valid ELF?
	if (ELFHDR->e_magic != ELF_MAGIC)
    7d29:	83 c4 0c             	add    $0xc,%esp
    7d2c:	81 3d 00 00 01 00 7f 	cmpl   $0x464c457f,0x10000
    7d33:	45 4c 46 
    7d36:	75 3f                	jne    7d77 <bootmain+0x64>
		goto bad;

	// load each program segment (ignores ph flags)
	ph = (proghdr *) ((uint8_t *) ELFHDR + ELFHDR->e_phoff);
    7d38:	8b 1d 1c 00 01 00    	mov    0x1001c,%ebx
	eph = ph + ELFHDR->e_phnum;
    7d3e:	0f b7 05 2c 00 01 00 	movzwl 0x1002c,%eax
	// is this a valid ELF?
	if (ELFHDR->e_magic != ELF_MAGIC)
		goto bad;
	...
	// load each program segment (ignores ph flags)
	ph = (proghdr *) ((uint8_t *) ELFHDR + ELFHDR->e_phoff);
    7d45:	81 c3 00 00 01 00    	add    $0x10000,%ebx
	eph = ph + ELFHDR->e_phnum;
    7d4b:	c1 e0 05             	shl    $0x5,%eax
    7d4e:	8d 34 03             	lea    (%ebx,%eax,1),%esi
	for (; ph < eph; ph++)
    7d51:	eb 14                	jmp    7d67 <bootmain+0x54>
		readseg(ph->p_va, ph->p_memsz, ph->p_offset);
    7d53:	ff 73 04             	pushl  0x4(%ebx)
    7d56:	ff 73 14             	pushl  0x14(%ebx)
    7d59:	ff 73 08             	pushl  0x8(%ebx)
		goto bad;
    ...
    7d5c:	83 c3 20             	add    $0x20,%ebx
		readseg(ph->p_va, ph->p_memsz, ph->p_offset);
    7d5f:	e8 70 ff ff ff       	call   7cd4 <readseg>
		goto bad;
	...
	eph = ph + ELFHDR->e_phnum;
	for (; ph < eph; ph++)
    7d64:	83 c4 0c             	add    $0xc,%esp
    7d67:	39 f3                	cmp    %esi,%ebx
    7d69:	72 e8                	jb     7d53 <bootmain+0x40>
		readseg(ph->p_va, ph->p_memsz, ph->p_offset);

	// call the entry point from the ELF header
	// note: does not return!
	((void (*)(void)) (ELFHDR->e_entry & 0xFFFFFF))();
    7d6b:	a1 18 00 01 00       	mov    0x10018,%eax
...
outw(int port, uint16_t data)
{
    ...
    7d8a:	eb fe                	jmp    7d8a <bootmain+0x77>
